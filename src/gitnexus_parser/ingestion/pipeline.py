...
def run_pipeline(
    ...
    *,
    ...
    All nodes are tagged with branch for (id, branch) uniqueness in Neo4j.
    ...
    process_structure(graph, paths_to_scan, branch=use_branch)
        ...
        try:
            ...
            try:
                ...
                if do_incremental and base_commit is not None and paths_to_scan != all_paths:
                    delete_nodes_by_file_paths(driver, write_branch, paths_to_scan, database=db)
                else:
                    delete_branch(driver, write_branch, database=db)
                write_graph(graph, driver, write_branch, database=db)
