...
def can_generate_children(conn, requirement_id: int) -> dict:
    ...
    return {
        ...
        "reason": None,
        "reason_code": None,
        "detail": f"?? {level.capitalize()} ??????????????? {child_level} ???",
    }
