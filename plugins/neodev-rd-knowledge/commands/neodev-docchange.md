...
- 先使用 `python plugins/neodev-rd-knowledge/validate_mvp_docs.py <docs_path>` 和 `python plugins/neodev-rd-knowledge/validate_obsidian_docs.py <docs_path>` 校验文档。
...
- 登记前先执行 `neodev doc import --doc-binding-id <doc_binding_id> --json`，确保目标文档返回可靠的 `last_seen_commit`。
