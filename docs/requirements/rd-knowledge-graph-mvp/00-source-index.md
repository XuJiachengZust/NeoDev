...
| SRC-U-011 | 对话确认 | 仓库图谱节点需要支持 AI 语义分析与向量化 | Repository AI Semantic Enrichment |
...
| SRC-U-014 | 对话确认 | 需要防止多分支之间相同代码被重复分析 | Cross-Branch Deduplication |
| SRC-U-015 | 对话确认 | 代码推送后需要支持更新链路上的代码节点和 AI 描述 | Post-Push Graph Refresh |
...
| SRC-C-007 | `src/service/routers/preprocess.py` | 已有按 `project_id + branch` 触发分析和查询状态的接口 | CLI / 服务接口映射 |
...
| SRC-C-011 | `src/service/services/llm_client.py` | 已有 chat / embedding probe 与 embedding 调用能力 | AI 语义增强 |
