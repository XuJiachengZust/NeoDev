...
- `CLI` 负责执行、校验、持久化、返回结构化事实

一期不做 UI，不做重工作流，对外统一提供 CLI，由本地 Codex、Claude Code、插件和 skill 调用。

## 3. MVP 目标

1. 让研发负责人以“产品 + 产品版本”视角组织代码、文档和知识关系。
2. 让文档变更成为代码实现闭环的起点。
3. 让本地智能工具在插件/skill 引导下调用 CLI，获取稳定事实和结构化图谱结果。
4. 让产品下任一项目仓库分支都可被显式触发分析，并可追踪状态与进度。
