...
| US-F001-02 | U-002 开发者或插件使用者 | 通过 `doc binding switch` 执行切换，并拿到目标绑定与后续 scan/import 指令 | 形成可自动化但不自动导入的文档工作流 | SRC-U-004, SRC-C-002, SRC-A-001 |
...
- 待切换的旧绑定可通过 `--doc-binding-id` 定位。（SRC-U-004, SRC-C-002）

## 6. 页面入口与导航路径

| 入口编号 | 页面/模块/能力入口 | 入口位置 | 触发方式 | 到达结果 | 来源编号/问题编号 |
| --- | --- | --- | --- | --- | --- |
| EN-F001-01 | CLI 文档绑定切换入口 | `doc binding switch` | 用户提交旧绑定 ID、目标版本、文档来源和分支 | 返回旧绑定删除结果、目标绑定、后续 scan/import 指令 | SRC-U-004, SRC-C-002, SRC-A-001 |
...
| BR-F001-05 | 后续扫描 | 切换命令不自动 scan；切换完成后，用户可显式对目标绑定执行 scan，写入目标版本作用域文档。 | BO-004-A01 | 无 | AC-F001-04 | SRC-U-004, SRC-C-004 | 已确认 |
| BR-F001-06 | 后续导入 | 切换命令不自动 import；切换完成后，用户可显式对目标绑定执行 import，写入目标版本文档图。 | BO-005 | 无 | AC-F001-05 | SRC-U-004, SRC-C-005, SRC-C-006 | 已确认 |
| BR-F001-07 | 清理 | 旧绑定删除时只清理绑定数据字段，不额外清理旧 documents、chunks、scan errors 或 Neo4j 文档图关系。 | BO-003 | 现有数据库约束导致的行为需在实现中如实处理 | AC-F001-06 | SRC-U-004 | 已确认 |

## 9. 字段、状态、枚举与校验

| 字段编号 | 字段名 | 所属对象属性 | 输入/展示 | 校验规则 | 错误提示 | 来源编号/问题编号 | 确认状态 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| FLD-F001-01 | `doc_binding_id` | BO-003-A01 | 输入 | 必须定位到旧 DocBinding | doc binding not found | SRC-U-004, SRC-C-002 | 已确认 |
| FLD-F001-02 | `version_name` 或 `version_id` | BO-002 | 输入 | 必须定位到旧绑定同一 Product 下的目标 ProductVersion | product version not found | SRC-U-004, SRC-C-002 | 已确认 |
...
flowchart LR
  ...
  Response --> ManualScanImport[用户显式 scan/import 目标绑定]
  ManualScanImport --> VersionScope[目标产品版本文档范围]
```
...
| 项 | 内容 | 来源编号/问题编号 | 确认状态 |
| --- | --- | --- | --- |
| 输入数据 | 旧 `doc_binding_id`、目标 ProductVersion 定位、目标文档来源、目标分支 | SRC-U-001, SRC-U-004, SRC-C-002 | 已确认 |
...
| 写数据边界 | 删除旧 DocBinding，创建目标 DocBinding；只清理绑定数据字段，不额外清理旧 documents、chunks、scan errors 或 Neo4j 文档图关系 | SRC-U-002, SRC-U-004 | 已确认 |
| 接口候选 | `doc binding switch --doc-binding-id <old_id> --version-name <target_version> --repo-path/--repo-url <source> --branch <branch> --json` | SRC-U-004 | 已确认 |
...
| AC-F001-06 | 切换已成功 | 检查切换副作用 | 旧绑定不可用；切换命令不自动 scan/import，也不额外清理旧 documents、chunks、scan errors 或 Neo4j 文档图关系 | BR-F001-02, BR-F001-05, BR-F001-06, BR-F001-07 | SRC-U-002, SRC-U-004 |

## 15. 测试场景建议

| 测试编号 | 优先级 | 场景 | 前置条件 | 操作 | 预期结果 | 覆盖 AC | 来源编号/问题编号 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| TC-F001-01 | P0 | CLI/API 切换命令注册 | 本地 CLI 可运行 | 查看 `doc binding switch --help` 或调用测试入口 | 命令/参数存在且输出结构稳定 | AC-F001-01 | SRC-T-002, SRC-U-004 |
...
| TC-F001-06 | P1 | 目标绑定导入写入文档图 | 切换成功且 Git 文档仓库可导入 | 对目标绑定显式执行 import 和 doc graph show | 文档图按目标 version 可查询 | AC-F001-05 | SRC-C-005, SRC-C-006 |
| TC-F001-07 | P0 | 切换不自动导入且只清理绑定数据字段 | 切换成功 | 检查命令响应和相关记录 | 响应无自动导入结果；未额外清理旧 documents、chunks、scan errors 或 Neo4j 文档图关系 | AC-F001-06 | SRC-U-004 |
...
| --- | --- | --- | --- | --- | --- | --- |
| 无 | Q-101 至 Q-104 已确认 | 无待确认问题 | 不适用 | 不适用 | 不适用 | 不适用 |
