...
| --- | --- | --- | --- | --- | --- |
| SRC-P-001 | 不适用 | 本期未提供 UI 原型，当前需求面向 CLI/API 能力 | 页面入口不纳入本期确定范围 | 若后续补 UI，需要单独确认页面入口与权限 | Q-101 |
...
| SRC-C-002 | `src/service/cli/commands/doc.py` | 当前 CLI 已有 `doc binding create` 和 `doc binding list`，没有显式切换或删除绑定命令 | 只能证明现有入口缺口，不能决定新增命令名称 | R-003, Q-102 |
...
| SRC-C-004 | `src/service/services/doc_scan_service.py` | 扫描绑定时根据 binding 的 `product_version_id` 写入 documents | 只覆盖扫描，不覆盖删除旧绑定后的图和 chunk 清理策略 | BR-F001-05, Q-103 |
| SRC-C-005 | `src/service/services/doc_import_service.py` | 导入绑定时根据 binding 的 `product_version_id` upsert 文档、写文档图、替换 chunk | 只覆盖导入，不确认切换时是否自动导入 | BR-F001-06, Q-104 |
...
| SRC-A-002 | `docker/init.sql` | `doc_bindings.product_version_id` 为产品版本外键，active 绑定存在版本和 Git 来源唯一性约束 | 具体删除级联和历史清理策略需实现时逐项验证 | BR-F001-04, Q-103 |
...
| SRC-T-004 | `tests/test_doc_import_service_unit.py` | 覆盖导入、chunk、version-scoped 文档 upsert | 若切换触发导入或清理，应补充导入侧回归测试 | TC-F001-05 |
